...
2. 打开右上角“开发者模式”。
3. 点击“加载已解压的扩展”，选择本目录 D:\番茄。
4. 打开或刷新一个番茄阅读页：https://fanqienovel.com/reader/*。
5. 修改文件后，在扩展管理页点击扩展卡片的刷新按钮，再刷新阅读页。

本项目没有构建步骤和运行时依赖，清单文件位于项目根目录。
...
- “同步邮件”会暂时隐藏邮箱外壳并提示用户点击番茄原生“目录”；扩展只观察原生目录 DOM，目录关闭后一次性读取真实章节列表。失败后只有用户明确点击“打开作品页同步”才进入作品页回退，不自动创建隐藏 iframe。
...
- Alt+Shift+M 切换皮肤；也可以点击工具栏中的“停用皮肤”。
- 点击“恢复番茄”会把同一个原生阅读盒子放回原位置，移除扩展外壳并恢复接管前的滚动位置。
- 换章整页导航开始时会先显示一次轻量的“正在加载邮件”过渡层；设置读取失败、停用、恢复、启动失败或超过 5 秒时会自动释放，不会遮挡原生目录同步。
- 启用皮肤时当前阅读标签显示固定标题“收件箱 - Outlook”，并使用本地打包的官方 Outlook 彩色 favicon；停用或恢复番茄时恢复当前文档最新的原生标题与图标。
...
- 目录同步依赖用户对官方阅读页原生目录的一次真实点击；目录结构、React 生命周期、页面结构、CSP、弹窗策略或作品页链接变化时会进入受控失败/显式回退状态，需由 Chrome/Edge 实机确认。
...
- M3 真实性能尚未量化；`data-fqmail-perf` 仅保留后续经批准批次可读取的非敏感样本，不把 `<=1s`/`<=100ms` 写成已达标。M4.3 的精确断点/启动异常和 M4.4 的实际解压加载、安装升级、刷新进度及非阅读页矩阵仍未执行。
